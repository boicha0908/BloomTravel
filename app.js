(() => {
  ...
  const COUNTRY_CURRENCIES = {
    '인도네시아': ['IDR', '인도네시아 루피아'], '일본': ['JPY', '일본 엔'], '중국': ['CNY', '중국 위안'],
    '대만': ['TWD', '대만 달러'], '홍콩': ['HKD', '홍콩 달러'], '마카오': ['MOP', '마카오 파타카'],
    '태국': ['THB', '태국 바트'], '베트남': ['VND', '베트남 동'], '필리핀': ['PHP', '필리핀 페소'],
    '말레이시아': ['MYR', '말레이시아 링깃'], '싱가포르': ['SGD', '싱가포르 달러'],
    '미국': ['USD', '미국 달러'], '캐나다': ['CAD', '캐나다 달러'], '호주': ['AUD', '호주 달러'],
    '뉴질랜드': ['NZD', '뉴질랜드 달러'], '영국': ['GBP', '영국 파운드'], '스위스': ['CHF', '스위스 프랑'],
    '프랑스': ['EUR', '유로'], '이탈리아': ['EUR', '유로'], '스페인': ['EUR', '유로'],
    '독일': ['EUR', '유로'], '포르투갈': ['EUR', '유로'], '그리스': ['EUR', '유로'],
    '터키': ['TRY', '터키 리라'], '두바이': ['AED', '아랍에미리트 디르함'], '아랍에미리트': ['AED', '아랍에미리트 디르함'],
    '인도': ['INR', '인도 루피'], '몰디브': ['MVR', '몰디브 루피야'], '괌': ['USD', '미국 달러'], '하와이': ['USD', '미국 달러']
  };
  const CITY_CURRENCIES = {
    '삿포로': ['JPY', '일본 엔'], 'sapporo': ['JPY', '일본 엔'], '도쿄': ['JPY', '일본 엔'], 'tokyo': ['JPY', '일본 엔'],
    '오사카': ['JPY', '일본 엔'], 'osaka': ['JPY', '일본 엔'], '교토': ['JPY', '일본 엔'], 'kyoto': ['JPY', '일본 엔'],
    '발리': ['IDR', '인도네시아 루피아'], '우붓': ['IDR', '인도네시아 루피아'], '괌': ['USD', '미국 달러'], '하와이': ['USD', '미국 달러']
  };
  const CURRENCY_DISPLAY_NAMES = { JPY: '엔화', IDR: '루피아', VND: '동', USD: '달러', EUR: '유로', CNY: '위안', TWD: '대만 달러', THB: '바트', SGD: '싱가포르 달러', GBP: '파운드', AUD: '호주 달러' };
  ...
  const countryCurrency = country => {
    ...
    const entry = Object.entries(COUNTRY_CURRENCIES).find(([name]) => value.includes(name.toLowerCase()));
    return entry ? { code: entry[1][0], name: entry[1][1] } : null;
  };
  const destinationCurrency = trip => countryCurrency(trip.country) || (trip.cities || []).map(city => {
    const value = String(city || '').trim().toLowerCase();
    const entry = Object.entries(CITY_CURRENCIES).find(([name]) => value.includes(name.toLowerCase()));
    return entry ? { code: entry[1][0], name: entry[1][1] } : null;
  }).find(Boolean) || null;
  const currencyDisplayUnit = code => ['JPY', 'IDR', 'VND'].includes(String(code).toUpperCase()) ? 100 : 1;
  const ensureTripCurrency = trip => {
    trip.budget = trip.budget || { planned: 0, baseCurrency: 'KRW', currencies: [] };
    if (!Array.isArray(trip.budget.currencies)) trip.budget.currencies = [];
    const local = destinationCurrency(trip);
    if (local) trip.budget.currencies = trip.budget.currencies.filter(item => item.manualRate || Number(item.rate || item.marketRate || 0) !== 0 || item.code === local.code);
    if (local && local.code !== 'KRW' && !trip.budget.currencies.some(item => item.code === local.code)) trip.budget.currencies.unshift({ code: local.code, name: local.name, rate: 0, marketRate: 0, manualRate: false });
    trip.budget.currencies.forEach(item => { if (item.manualRate == null) item.manualRate = false; if (item.marketRate == null) item.marketRate = Number(item.rate || 0); });
    return local;
  };
  const tripCurrencyCodes = trip => {
    ensureTripCurrency(trip);
    const local = destinationCurrency(trip)?.code;
    return [...new Set(['KRW', local, ...(trip.budget.currencies || []).map(item => item.code)].filter(Boolean))];
  };
  const currencyRate = (trip, code) => code === 'KRW' ? 1 : Number(trip.budget.currencies.find(item => item.code === code)?.rate || 0);
  ...
  const normalizeTrip = trip => {
    ...
    ensureTripCurrency(merged);
    const demoScheduleIds = new Set(['s1', 's2', 's3', 's4', 's5', 's6']);
    const inheritedDemo = merged.schedule.length > 0 && merged.schedule.every(entry => demoScheduleIds.has(entry.id));
    if (inheritedDemo && (merged.id !== 'trip-bali' || merged.country !== '인도네시아')) { merged.schedule = []; merged.bookings = []; merged.expenses = []; merged.comparisons = []; merged.diary = []; merged.places = []; merged.contacts = []; }
  ...
  function render() {
    ...
    if (activeView === 'planning') { main.innerHTML = planningView(trip); if (planningTab === 'budget') fetchExchangeRates(trip); }
    if (activeView === 'trip-mode') { main.innerHTML = tripModeView(trip); fetchExchangeRates(trip); }
    if (activeView === 'memories') main.innerHTML = memoriesView(trip);
  }

  async function fetchExchangeRates(trip) {
    const local = ensureTripCurrency(trip);
    const ratesNode = document.getElementById('rates-service');
    const currencies = trip.budget?.currencies || [];
    const codes = [...new Set(currencies.map(rate => String(rate.code).toUpperCase()).filter(code => code && code !== 'KRW'))];
    if (!codes.length) {
      if (ratesNode) ratesNode.innerHTML = '<span class="service-icon">₩</span><div><small>여행 통화</small><strong>대한민국 원화만 사용 중</strong></div>';
      return;
    }
    try {
      const ratesRes = await fetch(`https://api.frankfurter.dev/v2/rates?base=KRW&quotes=${codes.join(',')}`);
      const payload = await ratesRes.json(); const rows = Array.isArray(payload) ? payload : [];
      let values = Object.fromEntries(rows.map(row => [String(row.quote || '').toUpperCase(), Number(row.rate || 0)]));
      let updatedAt = rows[0]?.date || '';
      if (!ratesRes.ok || !Object.keys(values).length) {
        const legacyRes = await fetch(`https://api.frankfurter.dev/v1/latest?base=KRW&symbols=${codes.join(',')}`);
        const legacy = await legacyRes.json(); values = Object.fromEntries(Object.entries(legacy.rates || {}).map(([code, value]) => [code.toUpperCase(), Number(value)])); updatedAt = legacy.date || '';
      }
      if (!Object.keys(values).length) throw new Error('환율 응답이 비어 있습니다.');
      updatedAt = updatedAt || new Date().toISOString().slice(0, 10);
      let changed = false;
      currencies.forEach(item => {
        const inverse = Number(values[String(item.code).toUpperCase()] || 0);
        if (!inverse || !Number.isFinite(inverse)) return;
        const marketRate = 1 / inverse;
        if (Math.abs(Number(item.marketRate || 0) - marketRate) > 0.0000001 || item.rateUpdatedAt !== updatedAt) changed = true;
        item.marketRate = marketRate; item.rateUpdatedAt = updatedAt;
        if (!item.manualRate || !Number(item.rate)) item.rate = marketRate;
      });
      if (changed) saveState();
      const localItem = currencies.find(item => item.code === local?.code);
      const displayName = CURRENCY_DISPLAY_NAMES[local?.code] || local?.name || '현지 통화';
      const displayRate = localItem ? Number(localItem.rate || localItem.marketRate || 0) * currencyDisplayUnit(localItem.code) : 0;
      const displayText = displayRate > 0 ? `${displayName} ${displayRate.toFixed(2)}원` : `${displayName} 환율 조회 중`;
      if (ratesNode) ratesNode.innerHTML = `<span class="service-icon">₩</span><div><small>대한민국 원화 기준 · ${escapeHtml(updatedAt)}</small><strong>${escapeHtml(displayText)}</strong></div>`;
      const expenseCurrency = document.querySelector('#expense-form select[name="currency"]')?.value;
      const expenseRate = document.querySelector('#expense-form input[name="rate"]');
      if (expenseRate && expenseCurrency) expenseRate.value = currencyRate(trip, expenseCurrency) || 1;
    } catch (error) {
      if (ratesNode) ratesNode.innerHTML = `<span class="service-icon">₩</span><div><small>${escapeHtml(local?.name || '여행 통화')}</small><strong>환율을 불러오지 못했어요</strong></div>`;
    }
  }
  ...
  function dashboardView(trip) {
    ...
    const confirmedBookings = trip.bookings.filter(item => ['확정', '예약완료'].includes(item.status)).length;
    const bookingRatio = Math.round(confirmedBookings / Math.max(1, trip.bookings.length) * 100);
    const tripDays = Math.max(1, daysBetween(trip.start, trip.end));
    const scheduledDays = new Set(trip.schedule.map(item => item.date)).size;
    const scheduleRatio = Math.min(100, Math.round(scheduledDays / tripDays * 100));
    const diaryDays = new Set(trip.diary.map(item => item.date).filter(Boolean)).size;
    const diaryRatio = Math.min(100, Math.round(diaryDays / tripDays * 100));
    const shareRatio = trip.members.length > 1 ? 100 : 0;
    const bars = ['예약', '예산', '짐', '일정', '기록', '공유'].map((label, index) => ({ label, value: [bookingRatio, budget.ratio, packing.ratio, scheduleRatio, diaryRatio, shareRatio][index] }));
    ...
    return `${pageHeading(`BLOOM TRAVEL · ${trip.country}`, `${escapeHtml(names(trip))}의 <span style="color:var(--lilac-deep)">여행 준비</span>`, `${escapeHtml(trip.tagline || '해야 할 일은 가볍게, 기록은 오래 남도록.')}`, `<button class="button primary" data-action="quick-add">＋ 빠른 일정 추가</button>`)}
      ...
      <div class="two-col"><section class="panel"><div class="panel-heading"><div><h3>다가오는 일정</h3><p>예약과 시간을 한눈에 확인해요.</p></div><button class="link-btn" data-view="itinerary">전체 보기 →</button></div><div class="schedule-list">${upcoming.length ? upcoming.map(scheduleRow).join('') : '<div class="empty-state">다가오는 일정이 없어요.</div>'}</div></section><section class="panel"><div class="panel-heading"><div><h3>준비 기록</h3><p>예약·예산·준비물·일정·기록·공유 현황</p></div><span class="tag sage">진행 중</span></div><div class="mini-chart">${bars.map(bar => `<div class="mini-bar"><i style="height:${bar.value}%;min-height:${bar.value ? 7 : 0}px"></i><small>${bar.label}</small></div>`).join('')}</div><div class="mini-legend"><span>현재 여행 데이터 기준</span><span>여행 준비 흐름</span></div></section></div>
  ...
  function bookingManagerModal() { const trip = currentTrip(); openModal(`<button class="modal-close" data-action="close-modal">×</button><span class="eyebrow">CONNECTED BOOKINGS</span><h2>연결 예약 관리</h2><p>일정에 연결할 항공권·숙소·교통·투어 예약을 추가하거나 수정·삭제할 수 있어요.</p><div class="modal-actions" style="margin-top:0;margin-bottom:15px"><button class="button primary small" data-action="add-booking">＋ 예약 추가</button></div><div class="booking-grid">${trip.bookings.length ? trip.bookings.map(bookingCard).join('') : '<div class="panel empty-state">등록된 예약이 없어요. 예약을 추가해 주세요.</div>'}</div><div class="modal-actions"><button class="button ghost" data-action="close-modal">닫기</button></div>`); }
  ...
  async function saveTripForm(form) {
    ...
    try {
      ...
      if (item && previousCountry !== (data.country || '')) target.budget.currencies = target.budget.currencies.filter(currency => currency.manualRate);
      target.budget.currencyCountry = data.country || '';
      ensureTripCurrency(target);
      const demoScheduleIds = new Set(['s1', 's2', 's3', 's4', 's5', 's6']);
      const isInheritedDemoSchedule = target.schedule.length > 0 && target.schedule.every(entry => demoScheduleIds.has(entry.id));
      if (isInheritedDemoSchedule && (target.id !== 'trip-bali' || data.country !== '인도네시아')) { target.schedule = []; target.bookings = []; target.expenses = []; target.comparisons = []; target.diary = []; target.places = []; target.contacts = []; }
  ...
  function currencyModal() { const trip = currentTrip(); ensureTripCurrency(trip); openModal(`<button class="modal-close" data-action="close-modal">×</button><span class="eyebrow">CURRENCY TOOL</span><h2>환율 관리</h2><p>국가별 최신 기준 환율을 자동으로 반영하고, 필요한 경우 사용자 지정 환율을 입력할 수 있어요.</p><form id="currency-form" class="modal-form"><div class="field"><label>통화 코드</label><input name="code" placeholder="JPY" required /></div><div class="field"><label>통화 이름</label><input name="name" placeholder="일본 엔" required /></div><div class="field"><label>1단위당 원화 환율</label><input name="rate" type="number" step="0.0001" min="0" placeholder="9.2" required /></div><div class="modal-actions"><button class="button primary" type="submit">통화 추가</button></div></form><div class="rate-list">${trip.budget.currencies.map(rate => `<div class="rate-row"><span>${escapeHtml(rate.code)} · ${escapeHtml(rate.name)}${rate.manualRate ? ' · 사용자 지정' : ''}</span><strong>₩${Number(rate.rate || rate.marketRate || 0).toLocaleString('ko-KR')}</strong></div>`).join('')}</div>`); fetchExchangeRates(trip); }
  ...
  document.addEventListener('click', event => {
    ...
    if (action === 'save-trip') { saveTripForm(target.closest('form')); return; }
    ...
    if (action === 'delete-trip') { deleteTripById(target.dataset.id); return; }
    ...
    if (action === 'manage-bookings') { bookingManagerModal(); return; }
    ...
    if (action === 'quick-expense') { const trip = currentTrip(); openModal(`<button class="modal-close" data-action="close-modal">×</button><span class="eyebrow">QUICK EXPENSE</span><h2>빠른 지출 입력</h2><p>여행 중에는 필요한 항목만 빠르게 적어두세요.</p>${expenseForm(trip)}`); fetchExchangeRates(trip); return; }
  ...
  document.addEventListener('change', event => {
    if (event.target.dataset.rateCode) { const rate = currentTrip().budget.currencies.find(x => x.code === event.target.dataset.rateCode); if (rate) { rate.rate = Number(event.target.value || 0); rate.manualRate = true; saveState(); render(); showToast(`${rate.code} 환율을 저장했어요.`); } }
    if (event.target.matches('#expense-form select[name="currency"]')) { const input = document.querySelector('#expense-form input[name="rate"]'); if (input) input.value = currencyRate(currentTrip(), event.target.value) || 1; }
  });
  ...
  document.addEventListener('submit', async event => {
    ...
    if (form.id === 'trip-form') { await saveTripForm(form); return; }
    ...
    if (form.id === 'currency-form') { const code = data.code.toUpperCase(); if (!trip.budget.currencies.some(x => x.code === code)) trip.budget.currencies.push({ code, name: data.name, rate: Number(data.rate || 0), marketRate: Number(data.rate || 0), manualRate: true }); saveState(); closeModal(); render(); showToast(`${code} 통화를 추가했어요.`); return; }
  });
  ...
  if ('serviceWorker' in navigator && location.protocol !== 'file:') navigator.serviceWorker.register('sw.js?v=20260917-5').catch(() => {});
  initFirebase();
  render();
})();
