(() => {
  ...
  async function fetchExchangeRates(trip) {
    ...
    try {
      ...
      const updatedAt = rates.date || new Date().toISOString().slice(0, 10);
      let changed = false;
  ...
  if ('serviceWorker' in navigator && location.protocol !== 'file:') navigator.serviceWorker.register('sw.js?v=20260917-4').catch(() => {});
  initFirebase();
  render();
})();
