(() => {
  ...
  const ensureTripCurrency = trip => {
    ...
    const local = countryCurrency(trip.country);
  ...
  const tripCurrencyCodes = trip => {
    ...
    const local = countryCurrency(trip.country)?.code;
  ...
  async function fetchExchangeRates(trip) {
    ...
    try {
      ...
      const text = currencies.map(item => `${item.code} ₩${Number(item.rate || item.marketRate || 0).toFixed(item.code === 'IDR' || item.code === 'VND' ? 4 : 2)}`).join(' · ');
      if (ratesNode) ratesNode.innerHTML = `<span class="service-icon">₩</span><div><small>${escapeHtml(local?.name || '여행 통화')} · ${escapeHtml(updatedAt)} 기준</small><strong>${escapeHtml(text)}</strong></div>`;
